...
//    - the current live gold/silver rate (fetched from the same public APIs
//      the frontend TopBar ticker uses, with the same USD -> INR conversion
//      and the same "India premium" adjustment) so the price shown to the
//      customer always matches the rate shown on the website.
//
// Formula (per gram):
...
// Fetches (and caches) the live gold/silver rate in INR per gram.
const fetchLiveRates = async () => {
    ...
    try {
        ...
        const [goldData, silverData, currencyData] = await Promise.all([
