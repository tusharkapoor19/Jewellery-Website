...
// browser from the product's weight/metal and the live gold/silver rate that
// is already fetched & shown on the site (the TopBar ticker writes the final,
// already-converted INR/gram rate to localStorage — this reads that same
// value so the price shown always matches the rate shown).
//
// Formula (per gram):
